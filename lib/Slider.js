...
var propTypes = {
  ...
  countsPitPoints: _propTypes2['default'].object,
  ...
  _createClass(Rheostat, [{
    ...
    value: function () {
      function render() {
        ...
        var countPitPoints = _props4.countPitPoints;
        var ProgressBar = _props4.progressBar;


        var histo = [];
        if (PitComponent && countPitPoints) {
          var keysCountPitPoints = Object.keys(countPitPoints).map(Number

          // max height ?
          );var _max = 0;
          for (var i = 0; i < keysCountPitPoints.length; i++) {
            _max = _max < countPitPoints[String(i)].length ? countPitPoints[String(i)].length : _max;
          }

          // build each bar
          for (var _i = 0; _i < keysCountPitPoints.length; _i++) {
            var pos = _i * 5;
            var pitStyle = orientation === 'vertical' ? { top: pos + '%', position: 'absolute' } : { left: pos + '%', position: 'absolute' };
            var height = _max > 0 ? countPitPoints[String(_i)].length * 15 / _max : 0;
            pitStyle.height = height + 'px';
            histo.push(_react2['default'].createElement(
              PitComponent,
              { key: _i, style: pitStyle },
              pitStyle
            ));
          }
        }
